...
Example token values remain placeholders; DiscordChatExporter dependency and Discord data ownership are disclosed.
...
git diff --check: pass. python3 pyproject TOML parse + py_compile discord_exporter/*.py: pass. pytest not run because pytest is not installed locally.
